...
## Decorative background gears in the top-left / bottom-right corners.
@export var corner_gears_root: Control

const CORNER_GEAR_SPIN_DEGREES = 80.0
const CORNER_GEAR_SPIN_DURATION = 0.7
const CORNER_GEAR_FADE_DURATION = 0.35
const CORNER_GEAR_STAGGER = 0.05
...
func _animated_nodes() -> Array:
	var nodes: Array = [big_gear, small_gear_left, small_gear_right, hourglass, dim, title, buttons_root]
	nodes.append_array(_corner_gears())
	return nodes


func _corner_gears() -> Array:
	if corner_gears_root == null:
		return []
	return corner_gears_root.get_children()
...
func _start_intro() -> void:
	...
	var corners := _corner_gears()
	for i in corners.size():
		var corner_sign := 1.0 if i % 2 == 0 else -1.0
		_spin_corner_gear_in(tween, corners[i], corner_sign, CORNER_GEAR_STAGGER * i)
...
func _spin_corner_gear_in(tween: Tween, gear: Control, spin_sign: float, delay: float) -> void:
	var rest_rotation: float = _rest_rotations[gear]

	tween.tween_property(gear, "offset_transform_rotation", rest_rotation, CORNER_GEAR_SPIN_DURATION) \
			.from(rest_rotation + deg_to_rad(CORNER_GEAR_SPIN_DEGREES * spin_sign)) \
			.set_delay(delay) \
			.set_trans(Tween.TRANS_CUBIC).set_ease(Tween.EASE_OUT)
	tween.tween_property(gear, "modulate:a", 1.0, CORNER_GEAR_FADE_DURATION) \
			.from(0.0) \
			.set_delay(delay)
